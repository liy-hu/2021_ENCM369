...
void UserAppRun(void)
{
   /*Array of */
    ...
    u16 G_au16notes[] ={ C4, C4, G4, G4, A4, A4, G4, F4, F4, E4, E4, D4, D4, C4};
    u16 G_au16notetime[]={ N4, N4, N4, N4, N4, N4, N2, N4, N4, N4, N4, N4, N4, N2};
   
    
  /*Holds a single note for a specific duration specified in note duration array and starts song */
#if 0
    if(u16timecounter < G_au16notetime[u8notescounter])
    {
        InterruptTimerXus(G_au16notes[u8notescounter], true);    
        u16timecounter++;
        
    }
    
    /*After previous note plays,there will be no note played for RT amount of time  */
    else if(u16timecounter >= G_au16notetime[u8notescounter])
    {
        u16timecounter = 0; 
        ...
        /*updating index for next note*/
        u8notescounter++;
        if (u8notescounter == 14)
        {
            u8notescounter=0;
        }
